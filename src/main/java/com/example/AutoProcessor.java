...
public class AutoProcessor
{
    ...
    **/
    public static void process(Map configMap, BundleContext context)
    {
        ...
        logger.info("Felix : AUTO CONFIG = "+configMap.toString());
    ...
    private static void processAutoDeploy(Map configMap, BundleContext context)
    {
        ...
        if (actionList.size() > 0)
        {
            ...
            for (int i = 0; i < bundles.length; i++)
            {
                ...
                logger.info("Felix : Existing bundle detected = "+bundles[i].toString());
            }
            ...
            if (files != null)
            {
                ...
                for (int i = 0; i < files.length; i++)
                {
                    ...
                    {
                        jarList.add(files[i]);
                        logger.info("Felix : Auto deploy Bundle detected = "+files[i].toString());
                    }
                }
            }
            ...
            if (actionList.contains(AUTO_DEPLOY_UNINSTALL_VALUE))
            {
                for (Iterator it = installedBundleMap.entrySet().iterator(); it.hasNext(); )
                {
                    ...
                    if (b.getBundleId() != 0)
                    {
                        ...
                        {
                            b.uninstall();
                            logger.info("Felix : UNINSTALLED BUNDLE ("+b.toString()+")");
                        }
                        catch (BundleException ex)
                        {
            ...
            if (actionList.contains(AUTO_DEPLOY_START_VALUE))
            {
                for (int i = 0; i < startBundleList.size(); i++)
                {
                    try
                    {
                    	...
                    	bundle.start();
                        logger.info("Felix : STARTED BUNDLE ("+bundle.toString()+")");
                    }
                    catch (BundleException ex)
                    {
